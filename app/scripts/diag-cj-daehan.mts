// CJ대한통운 — account_id key fix 검증
// 실행: cd app && npx tsx scripts/diag-cj-daehan.mts

import { buildFinancialData, dumpStatement } from "./lib/diag-bootstrap.mts";

const CORP_CODE = "00113410";
const STOCK_CODE = "000120";
...
const checkBs = (rows: any[], label: string) => {
  dumpStatement(rows, `${label} BS`, YEARS, /부채|차입금|사채|리스/);
...
const verifyBsEquation = (rows: any[], label: string, year: string) => {
  const get = (kws: string[]) => {
    for (const r of rows) {
      ...
      if (kws.some((k) => n === k)) {
        return parseFloat(String(r[year] || "0").replace(/,/g, ""));
      }
    }
    return 0;
  };
